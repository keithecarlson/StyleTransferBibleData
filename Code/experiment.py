...
#Further patched to allow evaluation of old checkpoints
...
class Experiment(tf.contrib.learn.Experiment):
  ...
  def __init__(self, train_steps_per_iteration=None, *args, **kwargs):
    ...
    self.unevaluatedCheckpoints = []
    checkNums = []
    for file in os.listdir(self._estimator.model_dir):
      if '.index' in file:
        checkNums.append(int(file.split('.')[1].split('-')[1]))
    self._train_steps_per_iteration = train_steps_per_iteration
    for cn in sorted(checkNums):
      self.unevaluatedCheckpoints.append(os.path.join(self._estimator.model_dir,'model.ckpt-' + str(cn)))

    #print saver.latest_checkpoint(self._estimator.model_dir)
    #quit()
    #
  ...
  def evalAllCheckpoints(self):
    eval_result = None
    

    while(len(self.unevaluatedCheckpoints) > 0):
      checkName = self.unevaluatedCheckpoints[0]
      del self.unevaluatedCheckpoints[0]
